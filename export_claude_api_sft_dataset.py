"""Export Claude gateway session traces as conservative SFT dataset."""
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Export Claude gateway API traces as conservative SFT dataset."
    )
    ...
    parser.add_argument(
        "--include-incomplete",
        action="store_true",
        help="Allow extractor to retain incomplete/error requests before conversion",
    )
    parser.add_argument(
        "--continue-on-error",
        action="store_true",
