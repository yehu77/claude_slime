...
# Native-transformed tokenized SFT smoke for Qwen3-0.6B.
#
# This consumes the already-trimmed smoke_tokenized.jsonl directly. It does not
# re-export, re-prepare, or re-tokenize the native-transformed samples.
...
PREPARED_BUNDLE_DIR="${PREPARED_BUNDLE_DIR:-${CODEX_REPO}/outputs/native_transformed_sft/qwen_smoke_run_trim/train/smoke_tokenized.jsonl}"
TRAIN_OUTPUT_DIR="${TRAIN_OUTPUT_DIR:-${CODEX_REPO}/runs/slime_native_transformed_qwen3_0p6b_smoke}"
...
for path_var in CODEX_REPO SLIME_DIR MEGATRON_DIR MODEL_HF_DIR PREPARED_BUNDLE_DIR; do
  path_value="${!path_var}"
