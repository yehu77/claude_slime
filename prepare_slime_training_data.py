"""Prepare recommended slime training inputs from study/experiment outputs."""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Prepare a slime-compatible training bundle from pycodeagent outputs."
    )
    parser.add_argument("source_dir", help="Study, experiment, or batch output directory")
    ...
    parser.add_argument(
        "--include-failed",
        action="store_true",
        help="Include non-completed runs in the prepared dataset",
    )
    parser.add_argument(
        "--verifier-passed",
        choices=["true", "false", "any"],
        default="any",
        help="Filter on verifier outcome",
    )
    ...
    parser.add_argument("--run-id", default="slime_contract_train")
