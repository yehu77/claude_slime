...
def _apply_patch_handler(
    ...
    *,
    ...
    if not diff or not diff.strip():
        return ToolResult(ok=False, content="Empty diff", is_error=True)
    ...
    try:
        files_modified = _apply_unified_diff(diff, ctx)
    except _PatchError as exc:
        return ToolResult(
            ok=False,
            content=f"Patch failed: {exc}",
            is_error=True,
            metadata={"error_type": "patch_apply"},
        )
    except PathPolicyError as exc:
        return make_error_result(exc)
    except Exception as exc:
        return ToolResult(
            ok=False,
            content=f"Unexpected patch error: {exc}",
            is_error=True,
            metadata={"error_type": "patch_unexpected"},
        )
...
def _apply_unified_diff(diff: str, ctx: ToolContext) -> list[str]:
    ...
    while index < len(lines):
        ...
        files_modified.append(str(PurePosixPath(*rel_path.parts)))
