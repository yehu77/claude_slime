...
class SchemaCandidate:
    ...
    def __init__(
        ...
        *,
        ...
        """
        if category is not None and category not in SCHEMA_VARIANT_CATEGORIES:
            raise SchemaMutationError(
                "category must be one of "
                f"{sorted(SCHEMA_VARIANT_CATEGORIES)!r} or None, got {category!r}"
            )
    ...
    def from_dict(cls, data: dict[str, Any]) -> "SchemaCandidate":
        ...
        return cls(
            ...
            variant_id=(
                str(data["variant_id"])
                if data.get("variant_id") is not None
                else None
            ),
        )
