...
def run_study_from_config(
    ...
    *,
    ...
    Example:
        # With a real native-tools client (for production)
        from pycodeagent.agent.openai_native_client import OpenAINativeToolClient

        result = run_study_from_config(
            "configs/studies/first_mutation_sensitivity.json",
            client_factory=lambda: OpenAINativeToolClient(...),
        )
...
def run_study_from_provider_config(
    config_path: str | Path,
    provider_config: RuntimeProviderConfig | str | Path,
    *,
    output_dir: str | Path | None = None,
) -> StudyResult:
    """Run a study using a formal real-provider config."""
    ...
    return run_study_from_config(
        config_path,
        client_factory=build_llm_client_factory(resolved_provider_config),
        output_dir=output_dir,
    )


def run_study_with_provider_config(
    config: StudyConfig,
    provider_config: RuntimeProviderConfig | str | Path,
    *,
    output_dir: str | Path | None = None,
) -> StudyResult:
    """Run a study from a loaded StudyConfig using a real-provider config."""
    resolved_provider_config = (
        provider_config
        if isinstance(provider_config, RuntimeProviderConfig)
        else resolve_runtime_provider_config(provider_config)
