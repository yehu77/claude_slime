...
class TestSchemaMutationArgumentMapping:
    ...
    def test_argument_rename_create_file_maps_arguments_and_executes(
        self, registry: ToolRegistry, workspace: Path
    ):
        sampler = ToolProfileSampler(seed=0)
        profile = sampler.sample("argument_rename")
        runtime = ToolRuntime(registry)

        create_view = _get_tool_view(profile, "create_file")
        _, canonical_args = profile.map_call_arguments(
            create_view.exposed_name,
            {"file": "created.py", "text": "print('created')\n"},
            registry.get("create_file"),
        )
        assert canonical_args == {
            "path": "created.py",
        ...
        call = ToolCall(
            id="create_call",
            name=create_view.exposed_name,
            arguments={"file": "created.py", "text": "print('created')\n"},
        )
        result = runtime.execute(call, profile, _make_tool_context(workspace))
    ...
    def test_schema_mutated_write_file_maps_arguments_and_executes(
        self, registry: ToolRegistry, workspace: Path
    ):
        sampler = ToolProfileSampler(seed=0)
        profile = sampler.sample("schema_flat_to_nested")
        runtime = ToolRuntime(registry)

        write_view = _get_tool_view(profile, "write_file")
        exposed_args = {
            "target": {"path": "test.py"},
            "body": {"text": "print('updated')\n"},
        }
        _, canonical_args = profile.map_call_arguments(
            write_view.exposed_name,
            exposed_args,
        ...
        call = ToolCall(
            id="write_call",
            name=write_view.exposed_name,
            arguments=exposed_args,
        )
        result = runtime.execute(call, profile, _make_tool_context(workspace))

        assert result.ok
        assert (workspace / "test.py").read_text(encoding="utf-8") == "print('updated')\n"

    def test_schema_mutated_python_run_maps_arguments_and_executes(
        self, registry: ToolRegistry, workspace: Path
    ):
        sampler = ToolProfileSampler(seed=0)
        profile = sampler.sample("schema_flat_to_nested")
        ...
        python_view = _get_tool_view(profile, "python_run")
        exposed_args = {
            "execution": {
                "target": "test.py",
                "run_as_module": False,
        ...
        _, canonical_args = profile.map_call_arguments(
            python_view.exposed_name,
            exposed_args,
            registry.get("python_run"),
        )
        assert canonical_args == {
            "target": "test.py",
            "run_as_module": False,
...
class TestProfileModesProduceValidToolViews:
    ...
    def test_mode_exposes_all_builtin_tools(self, mode: str):
        """Each mode should expose all builtin tools."""
        sampler = ToolProfileSampler(seed=0)
        profile = sampler.sample(mode)
