...
real Claude Code
  ...
  -> prepared samples / tokenized.jsonl / train_config.json
```
...
This step intentionally reuses the existing Claude API SFT serializer and
loss-mask path. It does not introduce a new training sample format.
