"""Example Kilo Code wrapper that writes scaffold sidecar artifacts.
...
def main(argv: list[str] | None = None) -> int:
    ...
    target = workspace_dir / "counter.py"
    if not target.exists():
        target = workspace_dir / "KILO_WRAPPER_TOUCH.txt"

    original = target.read_text(encoding="utf-8") if target.exists() else ""
    target.write_text(original + "\n# Kilo wrapper touched this file.\n", encoding="utf-8")
        ...
        json.dumps(
            {
                ...
                "raw_payload": {},
                "parsed_payload": {"text": "I will inspect the repo and prepare a minimal change."},
                "parent_event_id": None,
                "artifact_refs": [],
                "error": None,
                "metadata": {"wrapper": "kilo_code_sidecar_wrapper"},
            }
        )
        + "\n"
        + json.dumps(
    ...
    raw_trace_summary_path.write_text(
        json.dumps(
            {
                "schema_version": 1,
                "trace_id": "kilo_code_wrapper_trace",
    ...
    print("kilo_code sidecar wrapper completed")
    return 0
